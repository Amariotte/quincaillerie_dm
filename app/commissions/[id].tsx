...
export default function CommissionDetailScreen() {
  ...
  useEffect(() => {
    const loadCommissions = async () => {
      try {
        const cachedCommissions = await getCacheData<listCommissions>(
          COMMISSIONS_LIST_CACHE_KEY,
        );
        const commission = cachedCommissions?.data.find(
          (item) => item.id === id,
        );
          ...
          >
            ...
            <View style={sharedStyles.metaRow}>
              <Text style={[sharedStyles.metaCaption, { color: mutedColor }]}>
                Date :{" "}
                {new Date(commission.dateCom).toLocaleDateString("fr-FR")}
              </Text>
              <Text style={[sharedStyles.metaCaption, { color: mutedColor }]}>
                Code de la vente : {commission.codeVente ?? "—"}
              </Text>
              <Text style={[sharedStyles.metaCaption, { color: mutedColor }]}>
                Date de la vente :{" "}
                {commission.dateVente
                  ? new Date(commission.dateVente).toLocaleDateString("fr-FR")
                  : "—"}
              </Text>
            </View>
          </View>
