...
export default function BonsScreen() {
  ...
  const filteredBons = bonLivraisons.data.filter((bon) => {
    const matchesQuery = bon.codeBL.toLowerCase().includes(query.toLowerCase());
    ...
    <SafeAreaView style={[sharedStyles.safeArea, { backgroundColor }]}>
      <View style={{ paddingHorizontal: 18, paddingTop: 12 }}>
        <AppHeader
          ...
          subtitle="Suivi des livraisons et des réceptions"
        />
      </View>
      <FlatList
          ...
          >
            ...
            <View style={sharedStyles.invoiceBottomRow}>
              <Text
                ...
                numberOfLines={1}
              >
                {bon.lieuLivraison?.trim()
                  ? `Lieu: ${bon.lieuLivraison}`
                  : "Lieu non renseigné"}
              </Text>
              <TouchableOpacity
                onPress={() => router.push(`/bons/${bon.id}` as never)}
