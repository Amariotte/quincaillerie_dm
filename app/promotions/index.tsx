...
export default function PromotionsScreen() {
  ...
  const [activeStatus, setActiveStatus] = useState<promotionStatus | "Tous">(
    "Tous",
  );
  ...
  const statusFilters = useMemo<(promotionStatus | "Tous")[]>(
    () => [
      "Tous",
      ...Array.from(
        new Set(
          promotions.data
            ...
            .filter(
              (status): status is promotionStatus =>
  ...
  const filteredPromotions = useMemo(() => {
    ...
    return promotions.data.filter((promotion) => {
      ...
      const matchesStatus =
        activeStatus === "Tous" || promotion.status === activeStatus;
  ...
  const activeCount = filteredPromotions.filter(
    (promotion) => promotion.status === "En cours",
  ).length;
  const upcomingCount = filteredPromotions.filter(
    (promotion) => promotion.status === "A venir",
  ).length;
    ...
    <SafeAreaView style={[sharedStyles.safeArea, { backgroundColor }]}>
      <View style={{ paddingHorizontal: 18, paddingTop: 12 }}>
        <AppHeader
          ...
          subtitle="Suivi des campagnes et disponibilités"
        />
      </View>
      <FlatList
        ...
        renderItem={({ item: promotion }) => {
          ...
          const statusColor =
            statusPromotionColorMap[promotion.status] || tintColor;
          const volumeLabel =
            promotion.nbMax > 0
          ...
          return (
            <TouchableOpacity
              activeOpacity={0.9}
              onPress={() =>
                router.push(`/promotions/${promotion.id}` as never)
              }
            ...
            >
              <View style={sharedStyles.invoiceTopRow}>
                <View style={styles.cardMainBlock}>
                  ...
                  <Text
                    style={[styles.productLabel, { color: mutedColor }]}
                    numberOfLines={1}
                  >
                  ...
                  >
                    {promotion.description ||
                      "Aucune description disponible pour cette campagne."}
                  </Text>
                </View>
                <View
                  style={[
              ...
              <View style={styles.quickFactsRow}>
                ...
                >
                  ...
                  <Text style={[sharedStyles.metaValue, { color: textColor }]}>
                    {promotion.dateFin ? formatDate(promotion.dateFin) : "—"}
                  </Text>
                </View>
          ...
          <View style={{ gap: 16 }}>
            <View style={sharedStyles.statsRow}>
              ...
              >
                ...
                <Text style={[sharedStyles.statCount, { color: "#16a34a" }]}>
                  {activeCount} promotion{activeCount > 1 ? "s" : ""}
                </Text>
              </View>
              ...
              >
                ...
                <Text style={[sharedStyles.statCount, { color: "#f59e0b" }]}>
                  {upcomingCount} promotion{upcomingCount > 1 ? "s" : ""}
                </Text>
              </View>
            </View>
        ...
        ListEmptyComponent={
          !showInitialLoader && !showErrorState ? (
            ...
            >
              ...
              <Text style={[sharedStyles.emptyText, { color: mutedColor }]}>
                Ajustez votre recherche ou la période.
              </Text>
            </View>
          ) : null
        }
