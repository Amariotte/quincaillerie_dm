...
const statusFilters: ("Toutes" | typeOperation)[] = [
  "Toutes",
  "Encaissement",
  "Décaissement",
];
...
export default function OperationsScreen() {
  ...
  const initialOperations = useMemo<listOperations>(
    () => ({
  ...
  const [activeStatus, setActiveStatus] = useState<"Toutes" | typeOperation>(
    "Toutes",
  );
  ...
  const filteredOperations = operations.data.filter((operation) => {
    ...
    const matchesStatus =
      activeStatus === "Toutes" || operation.libType === activeStatus;

    return matchesQuery && matchesDate && matchesClient && matchesStatus;
  });

  const encaissements = filteredOperations.filter(
    (operation) => operation.libType === "Encaissement",
  );
  const decaissements = filteredOperations.filter(
    (operation) => operation.libType === "Décaissement",
  );

  const totalCount = filteredOperations.length;
  const totalAmount = filteredOperations.reduce(
    (sum, operation) => sum + operation.montantOp,
    0,
  );
  const unsettledCount = encaissements.length;
  const unsettledAmount = encaissements.reduce(
    (sum, operation) => sum + operation.montantOp,
    0,
  );
  const overdueCount = decaissements.length;
  const overdueAmount = decaissements.reduce(
    (sum, operation) => sum + operation.montantOp,
    0,
  );
  const showInitialLoader = isLoading && operations.data.length === 0;
    ...
    <SafeAreaView style={[sharedStyles.safeArea, { backgroundColor }]}>
      <View style={{ paddingHorizontal: 18, paddingTop: 12 }}>
        <AppHeader
          ...
          subtitle="Suivi des opérations"
        />
      </View>
      <FlatList
        data={showInitialLoader || showErrorState ? [] : filteredOperations}
